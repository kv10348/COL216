...
void ID(){
	...
	if (deposit_ID_EX.IR.op == LW && (deposit_IF_ID.IR.rt==deposit_ID_EX.IR.rt||deposit_IF_ID.IR.rs==deposit_ID_EX.IR.rt)) //it is forwarding and lw
	{
        ...
        if((deposit_ID_EX.IR.op == ADD || deposit_ID_EX.IR.op == SUB ||deposit_ID_EX.IR.op==AND ||deposit_ID_EX.IR.op ==OR )&& (deposit_IF_ID.IR.op!=J) && (deposit_IF_ID.IR.op!=JAL)&&  (deposit_IF_ID.IR.rt==deposit_ID_EX.IR.rd||deposit_IF_ID.IR.rs==deposit_ID_EX.IR.rd)){
                stall_lw=1;
               // printf("%s%d%s\n","**********",deposit_IF_ID.IR.rt,"***********");
        }
...
void execute(struct inst_mem *im, int k, struct data_memory *dm){
       ...
       // int temp=0;
       printf("%s %d\n","===================================", label_pos("loop"));
        while(deposit_MEM_WB.pc!=global_pc){
